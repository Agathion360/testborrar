...
function cargaTabla(producto){
   ...
   return`
                    ...
                    <td class="opciones"> <button class="btn btn-info"><i class="fa-solid fa-eye"></i></button>   <button class="btn btn-warning"><i class="fa-solid fa-pen"></i></button>  <a class="btn btn-danger eliminar " id="${producto.id}"><i class="fa-solid fa-trash"></i></a> </td> `;
...
// buscador
function buscarMercaderia(e) {
    e.preventDefault()
    debugger
    let result = stock.filter((products)=> products.nombre.toLowerCase().includes(buscador.value.toLowerCase().trim()))
        if (result !== []) {
            cargarBusqueda(result)
        }
}
buscador.addEventListener("search", buscarMercaderia)
